...
app.get("/orders", async function (req, res) {
  dbConn.query(
    ...
    (SELECT TIMEDIFF(max(Updated), min(Updated)) as Elapsed from order_workflow_flow_steps where order_workflow_flow_steps.OrderID = ord.ID) as Elapsed,
    ...
    async function (error, orders, fields) {
      ...
      if (orders) {
        return res.send({
          error: false,
          data: orders,
          message: orders ? "List of orders" : "No order found",
        });
      }
    }
  );
...
const addChildLogsForOrder = async function (order, parentId, callback) {
  dbConn.query(
    "INSERT INTO logs SET ? ",
    {
      OrderID: order.ID,
      ParentID: parentId,
      LogName: `WESFC locating Inventory`,
      Created: getCurrentDate(),
    },
...
//                   await addStepLogs(orderId, res.insertId, WorkflowFlowID);
//                 }
//               );
//             }
//           );
...
const updateFlowLogs = async function (orderId, flowID, parentId, callback) {
  console.log("flowID==>", flowID, parentId);
        ...
        let nextPromise = async function () {
          ...
          if (i >= FlowResults.length) {
            await addOrderLogs(OrderID, async (lastLogId) => {
              ...
              setTimeout(async () => {
                await updateStepStatus(OrderID, lastLogId);
              }, 1000);
            });
            return callback({ status: "success" });
          } else {
...
const addStepLogs = async (orderId, stepID, lastLogId, Elapsed) => {
  dbConn.query(
    ...
    async (error, steps, fields) => {
      if (error) throw error;
      await insertLastLog(orderId, lastLogId, steps, Elapsed);
    }
  );
};
          ...
          await updateFlowStepStatus(
            ...
            async function (params) {
              if (j === wfResults.length - 1) {
                await updateWorkflowLogElapsed(
                  lastLogId,
                  fs.WorkflowID,
                  orderId
                );
...
const updateWorkflowLogElapsed = async function (
  lastLogId,
  WorkflowID,
  orderId
) {
...
const updateFlowStepStatus = async function (
  FlowID,
  orderId,
  lastLogId,
  j,
  ...
  await updateFlowLogs(
    orderId,
    FlowID,
    lastLogId,
    async function (lastLogForFlow) {
      dbConn.query(
...
const updateFlowElapsed = async function (
  orderId,
  flowID,
  stepID,
  lastLogForFlow
...
const updateOrderStepStatus = async function (
  orderId,
  owfsID,
  lastLogForFlow,
  stepID,
  ...
  dbConn.query(
    ...
    async (error, owfsIDResults, fields) => {
      if (error) throw error;
      let Elapsed = getTimeDifference(
        owfsIDResults[0].lastUpdatedTime,
        getCurrentDate()
      );
      ...
      console.log("curupdatedTime====>", getCurrentDate());
      dbConn.query(
...
const insertLastLog = async (orderId, lastLogId, steps, Elapsed) => {
  let logName = `WESFC send command to ${steps[0].StepName} for hardware ${steps[0].HardwareName}`;
...
app.get("/logs/:orderId", function (req, res) {
  ...
  let orderLogsArray = [];
  dbConn.query(
    `SELECT log.ID, log.LogName as name, log.ParentID, log.Created, log.Elapsed FROM logs as log
    where OrderId=? order by ID ASC`,
    orderId,
    function (error, logResults, fields) {
      ...
      if (logResults) {
        ...
        logResults.forEach((el) => {
          if (el.Elapsed && el.Elapsed === "00:00:00") {
            el.name = `${el.name}`;
          } else if (el.Elapsed) {
        ...
        return res.send({
          ...
          data: logs,
          message: logs
            ? "Order logs for order ID: " + orderId
